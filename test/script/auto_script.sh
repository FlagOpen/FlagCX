...
mpirun -np 8 ./test_broadcast -b 128M -e 2G -f 2 -p 1
if [ $? -ne 0 ]; then
    echo "test_broadcast execution failed!"
    exit 1
fi

mpirun -np 8 ./test_gather -b 128M -e 2G -f 2 -p 1
if [ $? -ne 0 ]; then
    echo "test_gather execution failed!"
    exit 1
fi

mpirun -np 8 ./test_scatter -b 128M -e 2G -f 2 -p 1
if [ $? -ne 0 ]; then
    echo "test_scatter execution failed!"
    exit 1
fi

mpirun -np 8 ./test_reduce -b 128M -e 8G -f 2 -p 1
if [ $? -ne 0 ]; then
