...
ifeq ($(USE_UCX), 1)
	...
	UCX_FLAG = -DHAVE_UCX_PLUGIN=1 -DUSE_UCX -DHAVE_IB_PLUGIN=1
	UCX_LINK = -L$(UCX_HOME)/lib -lucp -lucs -luct
else
	UCX_FLAG = -DHAVE_UCX_PLUGIN=0 -DHAVE_IB_PLUGIN=1
	UCX_LINK = 
endif
...
ifeq ($(USE_UCX), 0)
	LIBSRCFILES := $(filter-out flagcx/core/net_ucx_uct.cc flagcx/core/ucx_uct_lib.cc, $(LIBSRCFILES))
else
	# When UCX is enabled, all core files are included
	# p2p_plugin.cc is always needed as it manages plugin selection
endif
