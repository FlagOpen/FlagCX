...
FLAGCX_PARAM(IbMaxOutstanding, "IB_MAX_OUTSTANDING",
             128); // QP depth is 512, allow more for large messages
...
flagcxResult_t flagcxIbRetransInit(struct flagcxIbRetransState *state) {
  ...
  state->max_retry = flagcxParamIbRetransMaxRetry();
  state->ack_interval = flagcxParamIbRetransAckInterval();
  state->min_rto_us = flagcxParamIbRetransTimeout();
  state->max_rto_us = state->min_rto_us * 64;

  TRACE(FLAGCX_NET,
        "flagcxIbRetransInit: enabled=%d, max_retry=%d, ack_interval=%d, "
        "min_rto=%luus",
        state->enabled, state->max_retry, state->ack_interval,
        (unsigned long)state->min_rto_us);

  state->rto_us = state->min_rto_us;
  state->srtt_us = 0;
  state->rttvar_us = 0;
  ...
  state->last_ack_time_us = flagcxIbGetTimeUs();

  if (state->enabled) {
    INFO(FLAGCX_INIT | FLAGCX_NET,
         "IBUC Retransmission enabled: RTO=%uus, MaxRetry=%d, AckInterval=%d",
         state->min_rto_us, state->max_retry, state->ack_interval);
  }
...
flagcxResult_t flagcxIbRetransAddPacket(struct flagcxIbRetransState *state,
  ...
  if (state->buffer_count >= FLAGCX_IB_RETRANS_MAX_INFLIGHT) {
    static int warn_count = 0;
    if (warn_count++ < 5) {
      WARN("Retransmission buffer full: count=%d, max=%d. "
           "send_seq=%u, send_una=%u, recv_seq=%u, total_sent=%lu, "
           "total_acked=%lu",
           state->buffer_count, FLAGCX_IB_RETRANS_MAX_INFLIGHT, state->send_seq,
           state->send_una, state->recv_seq, (unsigned long)state->total_sent,
           (unsigned long)state->total_acked);
      WARN("This usually means ACK is not being received properly.");
      WARN("Dropping oldest packet to avoid deadlock (seq=%u)",
           state->buffer[state->buffer_head].seq);
    }

    state->buffer[state->buffer_head].valid = 0;
    state->buffer_head =
        (state->buffer_head + 1) % FLAGCX_IB_RETRANS_MAX_INFLIGHT;
    state->buffer_count--;
  }

  if (state->buffer_count > FLAGCX_IB_RETRANS_MAX_INFLIGHT * 3 / 4) {
    static uint64_t last_warn_time = 0;
    uint64_t now = flagcxIbGetTimeUs();
    if (now - last_warn_time > 1000000) {
      WARN("Retransmission buffer %d%% full (%d/%d). ACK may not be working "
           "properly.",
           (state->buffer_count * 100) / FLAGCX_IB_RETRANS_MAX_INFLIGHT,
           state->buffer_count, FLAGCX_IB_RETRANS_MAX_INFLIGHT);
      last_warn_time = now;
    }
  }

  int idx = state->buffer_tail;
  ...
  state->buffer_tail =
      (state->buffer_tail + 1) % FLAGCX_IB_RETRANS_MAX_INFLIGHT;
  state->buffer_count++;
  state->total_sent++;

  TRACE(FLAGCX_NET, "Added packet to retrans buffer: seq=%u, size=%u, count=%d",
        seq, size, state->buffer_count);
...
static void flagcxIbUpdateRTO(struct flagcxIbRetransState *state,
  ...
  if (state->srtt_us == 0) {
    ...
    state->rttvar_us = rtt_us / 2;
  } else {
    // RFC 6298: SRTT = (1 - alpha) * SRTT + alpha * RTT
    // RTTVAR = (1 - beta) * RTTVAR + beta * |SRTT - RTT|
    // alpha = 1/8, beta = 1/4
    uint64_t delta = (rtt_us > state->srtt_us) ? (rtt_us - state->srtt_us)
                                               : (state->srtt_us - rtt_us);
    state->rttvar_us = (3 * state->rttvar_us + delta) / 4;
    state->srtt_us = (7 * state->srtt_us + rtt_us) / 8;
  }

  state->rto_us = state->srtt_us + 4 * state->rttvar_us;

  if (state->rto_us < state->min_rto_us) {
    state->rto_us = state->min_rto_us;
  }
  if (state->rto_us > state->max_rto_us) {
    state->rto_us = state->max_rto_us;
  }

  TRACE(FLAGCX_NET,
        "Updated RTO: RTT=%luus, SRTT=%luus, RTTVAR=%luus, RTO=%luus",
        (unsigned long)rtt_us, (unsigned long)state->srtt_us,
        (unsigned long)state->rttvar_us, (unsigned long)state->rto_us);
}
...
flagcxResult_t flagcxIbRetransProcessAck(struct flagcxIbRetransState *state,
  ...
  uint32_t ack_seq = ack_msg->ack_seq;
  uint64_t now_us = flagcxIbGetTimeUs();

  TRACE(FLAGCX_NET, "Processing ACK: ack_seq=%u, send_una=%u, buffer_count=%d",
        ack_seq, state->send_una, state->buffer_count);

  if (ack_msg->timestamp_us > 0) {
    uint64_t rtt_us = now_us - ack_msg->timestamp_us;
    if (rtt_us < 10000000) {
      flagcxIbUpdateRTO(state, rtt_us);
    }
  }

  state->last_ack_time_us = now_us;

  int freed = 0;
  while (state->buffer_count > 0) {
    struct flagcxIbRetransEntry *entry = &state->buffer[state->buffer_head];
    ...
    if (!entry->valid) {
      ...
      state->send_una = entry->seq + 1;
    } else {
      break;
    }
  }

  uint64_t sack_bitmap = ack_msg->sack_bitmap;
  uint16_t sack_count = ack_msg->sack_bitmap_count;
  ...
  if (sack_bitmap != 0 && sack_count > 0) {
    ...
    for (int i = 0; i < state->buffer_count && i < 64; i++) {
      struct flagcxIbRetransEntry *entry = &state->buffer[idx];

      if (entry->valid) {
        uint32_t entry_offset = entry->seq - state->send_una;
...
flagcxResult_t flagcxIbDestroySrq(struct flagcxIbSrqMgr *srqMgr) {
  ...
  for (int i = 0; i < srqMgr->buf_count; i++) {
    if (srqMgr->bufs[i].mr) {
...
flagcxResult_t flagcxIbSrqPostRecv(struct flagcxIbSrqMgr *srqMgr, int count) {
  ...
  if (srqMgr->free_buf_count == 0 || srqMgr->post_srq_count == 0) {
    return flagcxSuccess;
  }
  ...
  if (post_batch > srqMgr->post_srq_count)
    post_batch = srqMgr->post_srq_count;
  if (post_batch > srqMgr->free_buf_count)
    post_batch = srqMgr->free_buf_count;
  ...
  for (int i = 0; i < post_batch; i++) {
    ...
    srqMgr->free_buf_count--;
    int buf_idx = srqMgr->free_buf_indices[srqMgr->free_buf_count];
  ...
  srqMgr->post_srq_count -= post_batch;

  TRACE(FLAGCX_NET,
        "Posted %d recv WRs to SRQ (free_buf_count=%d, post_srq_count=%d)",
        post_batch, srqMgr->free_buf_count, srqMgr->post_srq_count);
